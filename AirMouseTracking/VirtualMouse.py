...
anchocam, altocam = 720,480
cuadro = 100
user32 = ctypes.windll.user32
user32.SetProcessDPIAware()
anchopan, altopan = user32.GetSystemMetrics(0), user32.GetSystemMetrics(1)
""" anchopan, altopan = autopy.screen.size() """
sua = 5
pubix, pubiy = 0,0
cubix, cubiy = 0,0
...
while True:
	...
	if len(lista) !=0:
		...
		if dedos[1]== 1 and dedos[2]==0:
			...
			x3= np.interp(x1,(cuadro,anchocam-cuadro),(0,anchopan))
			y3= np.interp(y1,(cuadro,altocam-cuadro),(0,altopan))

			cubix = pubix + (x3 - pubix) / sua
			cubiy = pubiy + (y3 - pubiy) / sua

			autopy.mouse.move(anchopan - cubix,cubiy)
